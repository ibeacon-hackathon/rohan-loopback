...
module.exports = function(app) {
    app.dataSources.listDB.automigrate('List', function(err) {
        if(err) throw err;
        app.models.List.create(initDB.lists, function(err, lists) {
            if(err) throw err;
            console.log('Models created:', lists);
        });
    });
    app.dataSources.taskDB.automigrate('Task', function(err) {
        if(err) throw err;
        app.models.Task.create(initDB.tasks, function(err, tasks) {
            if(err) throw err;
            console.log('Models created:', tasks);
        });
    });
    app.dataSources.userDB.automigrate('User', function(err) {
        if(err) throw err;
        app.models.User.create(initDB.users, function(err, users) {
            if (err) throw err;
            console.log('Models created:', users);
        });
    });
};
